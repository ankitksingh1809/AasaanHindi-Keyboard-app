...
import com.example.keyboard.Keyboard.Key;
...
//        void swipeUp();
    ...
    private Key[] mKeys;
    private int mPopupx1;
    private int mPopupy1;
...
//                if(event.getAction() == MotionEvent.ACTION_UP) {
                    if (mMiniKeyboardOnScreen) {
                        if (isViewInBounds(mMiniKeyboard, x, y)) {
                            mMiniKeyboard.dispatchTouchEvent(event);
                        }
                        else if (isViewInBounds(keyboardView, x, y)) {
...
//        onSizeChanged(this.widtzsc,this.getMeasuredHeight(),this.getMeasuredWidth(),this.getMeasuredHeight());
//        mKeyboard.resize(this.getWidth(),this.getHeight());
//        requestLayout();
//        invalidateAllKeys();
//        computeProximityThreshold(keyboard);
//        mMiniKeyboardCache.clear();
    ...
    public int getmPopupx1() {
        return mPopupx1;
    }

    public int getmPopupy1() {
        return mPopupy1;
    }
    ...
    private void onBufferDraw() {
        ...
        canvas.drawColor(Color.rgb(213,217,222));
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            if (label != null) {
                if(key.codes[0]==-2 || key.codes[0]==-7){
                    ...
                    Drawable keyBackground1 = keyBackground;
                    keyBackground1.setTint(Color.rgb(152,158,166));
                    keyBackground1.draw(canvas);
                    keyBackground1.setTint(Color.rgb(255,255,255));
                } else if ((label.length() > 1 || key.text!=null) && (!label.equals("क्ष"))) {
                    paint.setTextSize(mKeyTextSize*4/5);
                    paint.setTypeface(Typeface.DEFAULT);
//                    paint.setTextSize(mLabelTextSize*(3)/2);
//                    paint.setTypeface(Typeface.DEFAULT_BOLD);
                } else {
                    if(label.equals("⌫") || label.equals(",") || label.equals("।") || label.equals(".")) {
                        paint.setTextSize(mKeyTextSize*8/7);
                        paint.setTypeface(Typeface.DEFAULT);
                        Drawable keyBackground1 = keyBackground;
                        keyBackground1.setTint(Color.rgb(152,158,166));
                        keyBackground1.draw(canvas);
                ...
                paint.setShadowLayer(0, 0, 0, 0);
                 ...
                 if (key.codes[0]==10) {
                    paint.setTextSize(mKeyTextSize*3/2);
                    paint.setTypeface(Typeface.DEFAULT_BOLD);
                    Drawable keyBackground1 = keyBackground;
                    keyBackground1.setTint(Color.rgb(54,86,191));
                    keyBackground1.draw(canvas);
                    keyBackground1.setTint(Color.rgb(255,255,255));
                }
        ...
        if (popupKeyboardId != 0) {
            ...
            if (mMiniKeyboardContainer == null) {
                ...
                mMiniKeyboard = mMiniKeyboardContainer.findViewById(
                        R.id.PopUpKeyboardView);
                View closeButton = mMiniKeyboardContainer.findViewById(R.id.closeButton);
                if (closeButton != null) closeButton.setOnClickListener(this);
                mMiniKeyboard.setOnKeyboardActionListener(new PopUpKeyboardView.OnKeyboardActionListener() {
...
//                        mKeyboardActionListener.onPress(primaryCode);
                ...
                mMiniKeyboard = mMiniKeyboardContainer.findViewById(R.id.PopUpKeyboardView);
            }
            getLocationInWindow(mCoordinates);
            mPopupX = popupKey.x + getPaddingLeft();
            mPopupY = popupKey.y + getPaddingTop();
            final int x = mPopupX + popupKey.width/2 - mMiniKeyboardContainer.getMeasuredWidth()/2 + mCoordinates[0];
            final int y = mPopupY + popupKey.height/2 - mMiniKeyboardContainer.getMeasuredHeight()/2 + mCoordinates[1];
            mPopupx1 = x - mCoordinates[0];
            mPopupy1 = y - mCoordinates[1];
...
//         displayed the mini keyboard
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if(keyIndex != NOT_A_KEY){
                    if(mKeys[keyIndex].text != null){
                        mKeyboardActionListener.onText(mKeys[keyIndex].text);
                    }else if (mKeys[keyIndex].codes[0]!=-5) {
