...
public class Keyboard {
    ...
    public static final int KEYCODE_MODE_CHANGE1 = -7;
...
//        mModifierKeys = new ArrayList<>();
//        mKeyboardMode = 0;
//        loadKeyboard(mContext, mXml);
        int numRows = rows.size();
//        int y=0;
//        float scaleFactor1 = (float) (newHeight - (rows.get(0).verticalGap*7)) / (rows.get(0).defaultHeight*7);
...
//                    key.height *= scaleFactor1;
                    key.x = x;
//                    key.y = y;
                    x += key.width + key.gap;
                }
            }
//            y += row.mKeys.get(0).height + row.verticalGap;
        }
            ...
            while ((event = parser.next()) != XmlResourceParser.END_DOCUMENT) {
                if (event == XmlResourceParser.START_TAG) {
                    ...
                    if (inKey) {
                        ...
                        if(key.codes[0]==32){
                            key.width*=5.1;
                        } else if(rows.size()==4 && key.codes[0]==-7){
                            key.width*=(1.8);
                        }
                        else if(currentRow.mKeys.size()==1 || currentRow.mKeys.size()==10){
                            key.width*=(1.6);
                        } else{
                            if(rows.size()==6 && currentRow.mKeys.size()==5){
                                key.width*=(1.6);
                                ...
                                margin = (int)(key.width*(1.2));
                                key.width = (int) (key.width * (6.8)) / 8;
                            }
                        }
                        if(rows.size()==6){
                            key.height = margin;
                        ...
                        inRow = false;
                        if(rows.size()==6){
                            currentRow.defaultHeight = margin;
                        }
